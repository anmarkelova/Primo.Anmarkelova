...
{
    public class FullWeatherData : PrimoComponentTO<Design.CityPressure_Form>
    {
        private const string GROUP_NAME = "CustomLib" + WFPublishedElementBase.TREE_SEPARATOR + "Погода OpenWeatherMap",
                             ...
                             ACTIVITY_NAME = "Погода в городе",
                             ACTIVITY_DESCRIPTION = "Активность позволяет получить полную информацию о погоде в указанном городе.",
        ...
        /// <summary>
        /// Свойство, которое хранит информацию о погоде в городе.
        /// </summary>
        [LTools.Common.Model.Serialization.StoringProperty]
        [System.ComponentModel.Category(OUTPUT_CATEGORY), System.ComponentModel.DisplayName("Погода")]
        public string Weather
        {
            get { return this.weather; }
            set { this.weather = value; this.InvokePropertyChanged(this, nameof(Weather)); }
        }
        #endregion


        #region [PROPERTIES][TYPES]
        public FullWeatherData(IWFContainer container) : base(container)
        {
            ...
            sdkComponentHelp += "\r\n"
                             + nameof(Weather) + "*: Погода в городе.";
            ...
            sdkProperties = new List<LTools.Common.Helpers.WFHelper.PropertiesItem>()
            {
                ...
                new LTools.Common.Helpers.WFHelper.PropertiesItem()
                {
                    PropName = nameof(Weather),
                    PropertyType = LTools.Common.Helpers.WFHelper.PropertiesItem.PropertyTypes.VARIABLE,
                    EditorType = ScriptEditorTypes.NONE,
                    DataType = typeof(string), ToolTip = "[string] Погода в городе по данным OpenWeatherMap в JSON формате.", IsReadOnly = false
                },
            };
            InitClass(container);
        }
        ...
        public override ExecutionResult TimedAction(ScriptingData sd)
        {
            try
            {
                ...
                var response = new WeatherData().GetFullWeatherJson(key, chosenCity);
